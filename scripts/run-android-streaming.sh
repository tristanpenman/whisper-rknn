...
# Push the Android build of whisper-rknn-streaming to a device and run it.
#
# Usage:
#   ./scripts/run-android-streaming.sh <device-ip> [model-dir] [rknn-decoder] [rknn-encoder] [sample-path]
#
# Arguments:
...
WHISPER_RKNN_BIN="${BUILD_DIR}/whisper-rknn-streaming"
...
# Push binaries
"${ADB[@]}" push "${WHISPER_RKNN_BIN}" "${REMOTE_DIR}/whisper-rknn-streaming"
...
"${ADB[@]}" shell chmod 755 "${REMOTE_DIR}/whisper-rknn-streaming"
...
echo
echo "=== Start whisper-rknn-streaming on ${SERIAL} ==="
...
"${ADB[@]}" shell -t -t \
  ...
  ./whisper-rknn-streaming \
  --enable-timestamps \
  ${REMOTE_MODEL_DIR}/encoder.rknn \
  ${REMOTE_MODEL_DIR}/decoder.rknn \
  en \
  ${REMOTE_DIR}/sample.wav \
  2>&1"
