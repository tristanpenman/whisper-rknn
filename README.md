...
Pass `--disable-neon` to use the scalar Mel-spectrogram matrix multiplication implementation. This is useful for comparing its output with the default Arm NEON implementation. Pass `--enable-timestamps` to include Whisper segment timestamp markers. The chunk length and decoder's fixed token input length are read from the RKNN model tensor dimensions at load time, so neither needs to be passed on the command line.
